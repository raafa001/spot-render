...
> ℹ️ **Dica:** se estiver rodando o SonarQube localmente (ex.: WSL2), edite `sonar.properties` para incluir `sonar.web.host=0.0.0.0` e use o endereço acessível pelo runner self-hosted (por exemplo `http://127.0.0.1:9000` ou o IP do gateway) antes de salvar `SONAR_HOST_URL` nos secrets.
